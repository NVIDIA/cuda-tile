...
class TestFindTools:
    def test_explicit_cuda_tile_translate(self, tmp_path):
        ...
        with patch("basic_tile.runner._search_paths") as mock_search:
    ...
    def test_missing_cuda_tile_translate_raises(self):
        with patch("basic_tile.runner._search_paths", return_value=[]):
...
class TestCompileMlirToTilebc:
    def test_success(self, tmp_path):
        ...
        with patch("basic_tile.runner.subprocess.run", side_effect=fake_run):
    ...
    def test_failure_nonzero_exit(self, tmp_path):
        ...
        with patch("basic_tile.runner.subprocess.run") as mock_run:
...
class TestDetectGpuArch:
    ...
    def test_nvidia_smi_missing(self):
        with patch(
            "basic_tile.runner.subprocess.run", side_effect=FileNotFoundError
        ):
...
class TestLaunchCubin:
    def test_driver_api_sequence(self, tmp_path):
        ...
        with patch("basic_tile.runner.ctypes.CDLL", return_value=mock_libcuda):
            launch_cubin(cubin)
    ...
    def test_libcuda_not_found(self, tmp_path):
        ...
        with patch("basic_tile.runner.ctypes.CDLL", side_effect=OSError("not found")):
...
class TestCompileAndRun:
    def test_compile_only(self, tmp_path):
        ...
        with (
            patch("basic_tile.runner.find_tools", return_value=tools),
            patch("basic_tile.runner.detect_gpu_arch", return_value="sm_120"),
            patch("basic_tile.runner.subprocess.run", side_effect=fake_run),
        ):
    ...
    def test_full_run(self, tmp_path):
        ...
        with (
            ...
            patch("basic_tile.runner.detect_gpu_arch", return_value="sm_120"),
            patch("basic_tile.runner.subprocess.run", side_effect=fake_run),
            patch("basic_tile.runner.launch_cubin") as mock_launch,
        ):
            result = compile_and_run(
